#!/usr/bin/env python3
"""
Port API Integration Script
Automated remediation for notification-service owner assignment

This script uses Port's REST API directly to update the entity when MCP is unavailable.
"""
...
class PortAPIClient:
    """Simple Port API client for entity operations"""
    ...
    def list_teams(self) -> Optional[list]:
        """List available teams (if team blueprint exists)"""
        try:
            response = requests.get(
...
def print_banner():
    """Print script banner"""
    print("=" * 60)
    print("Port API - Notification Service Owner Remediation")
    print("=" * 60)
    print()
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 port_api_fix.py <team-identifier>")
        print()
        print("Example:")
        print("  python3 port_api_fix.py platform")
        print()
        print("Available team identifiers (common examples):")
        print("  - platform")
        print("  - backend-team")
        print("  - notifications-team")
    ...
    if success:
        ...
        if updated_entity:
            ...
            if new_owner == team_identifier:
                ...
                print()
                print("Next steps:")
                print("1. Check scorecard to verify rule passes")
                print("2. Monitor for persistence after sync cycles")
                print("3. Document the change in your records")
            else:
